...
parser.add_argument('--submit_data', default='test_submission_task_1.csv', type=str)
...
submission = pd.merge(true_ids, submission, on=['UserId','QuestionId'])[['UserId','QuestionId','IsCorrect']]
...
# compute scores
score = np.sum(np.array(submission.IsCorrect)==np.array(truth.IsCorrect)) / float(len(submission.IsCorrect))
conf_mtx = confusion_matrix(np.array(truth.IsCorrect), np.array(submission.IsCorrect)) #/ len(submission.IsCorrect)
conf_mtx = conf_mtx.astype('float') / conf_mtx.sum(axis=1).astype('float')
pred = ['pred_0', 'pred_1']
true = ['true_0', 'true_1']
...
# https://github.com/codalab/codalab-competitions/wiki/User_Building-a-Scoring-Program-for-a-Competition#directory-structure-for-submissions
with open(os.path.join(output_dir, 'scores_task_1.txt'), 'w') as output_file:
    output_file.write("score:{0}\n".format(score))

# output detailed results
with open(os.path.join(output_dir, 'scores_task_1.html'), 'w') as output_file:
    htmlString = '''<!DOCTYPE html>
                    <html>
                    <p>phase 1: classification accuracy, predict right or wrong</p>
                    </br>
