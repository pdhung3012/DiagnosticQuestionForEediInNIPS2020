...
parser.add_argument('--output_file', default=os.path.normpath('submission_task_1.csv'), type=str)
...
# parser.add_argument('--output_file', default=os.path.normpath('test_submission_task_1.csv'), type=str)
...
top_answers = df.groupby('QuestionId')['IsCorrect'].agg(lambda x:x.value_counts().index[0])
top_answers_dict = top_answers.to_dict()

test_data_path = os.path.join(submission_template_dir, submission_template_file)
test_df = pd.read_csv(test_data_path)

test_df['IsCorrect'] = test_df['QuestionId'].map(top_answers_dict)
